  ...
  <main>
    <header class="hero" id="intro">
      ...
      <div class="note">
        <span class="ic">🧭</span>
        <div>
          <p><strong>La app de ejemplo.</strong> El manual usa nombres genéricos <code>miorg/miapp</code> para que los adaptes. El flujo es idéntico para cualquier stack: cambia solo el contenido del <code>Dockerfile</code> y del esqueleto que pedís en el paso 2.</p>
        </div>
      </div>
    ...
    <section class="step" id="s1">
      ...
      <ol class="steps">
        <li>Entrá a <a href="https://github.com/new" target="_blank" rel="noopener">github.com/new</a>.</li>
        <li>Nombre: <code>miapp</code>. Visibilidad: <strong>Private</strong> (el deploy key funciona igual con repos privados).</li>
        <li>Tildá <strong>Add a README</strong> y elegí <strong>.gitignore → nada aún</strong> (lo agregamos nosotros). Create repository.</li>
      </ol>
...
.DS_Store</code></pre>
      ...
      <div class="note">
        <span class="ic">🔑</span>
        <div><p>Anotá la <strong>URL SSH</strong> del repo: <code>git@github.com:miorg/miapp.git</code>. La vas a necesitar en el paso 4 para conectar EasyPanel.</p></div>
      </div>
    </section>
    ...
    <section class="step" id="s2">
      ...
      <p>En vez de crear archivos a mano, le pedís a Claude que genere el esqueleto completo describiendo el <strong>stack</strong> y las <strong>convenciones</strong>. Cuanto más precisa la especificación, menos correcciones después.</p>

      <h3>Prompt plantilla (copiar y completar)</h3>
      <p>Reemplazá lo que está entre <code>«…»</code>. Este ejemplo ya viene cargado con el stack del manual:</p>
      <figure class="code" data-lang="prompt">
        ...
        <pre><code>Armá el esqueleto base de una aplicación web lista para deploy con Docker.
...
Devolveme cada archivo con su ruta y su contenido completo.</code></pre>
      </figure>
...
└── .gitignore</div>
      ...
      <p>Revisá lo generado, hacé commit y push a <code>main</code>:</p>
      <figure class="code" data-lang="bash">
        <figcaption><span class="fname"><span class="dot"></span>terminal</span><button class="copybtn" type="button">Copiar</button></figcaption>
        <pre><code>git add .
...
git push <span class="tok-k">-u</span> origin main</code></pre>
    ...
    <section class="step" id="s3">
      ...
      <p>En EasyPanel un <strong>Proyecto</strong> agrupa servicios (tu app + su base de datos). Dentro creás un servicio de tipo <strong>App</strong>.</p>
      <ol class="steps">
        <li>En el dashboard, <strong>Create Project</strong> → nombre <code>miapp</code>.</li>
        <li>Dentro del proyecto, <strong>+ Service</strong> → elegí <strong>App</strong>.</li>
        <li>Ponele nombre al servicio, ej. <code>web</code>. Queda como <code>miapp_web</code> a nivel interno.</li>
        <li>Guardá. Todavía no despliega nada — falta conectar la fuente (paso 4) y definir el build (paso 5).</li>
      </ol>
      <div class="note">
    ...
    <section class="step" id="s4">
      ...
      <ol class="steps">
        <li>En el servicio <code>web</code>, pestaña <strong>Source</strong> → elegí <strong>GitHub</strong> (modo Deploy Key, no la GitHub App).</li>
        <li>Repository: <code>git@github.com:miorg/miapp.git</code> · Branch: <code>main</code>.</li>
        <li>EasyPanel genera y muestra una <strong>Deploy Key</strong> (clave pública SSH). Copiala entera.</li>
      </ol>

      <h3>4.2 · Cargar la deploy key en GitHub</h3>
      <ol class="steps">
        <li>En el repo → <strong>Settings → Deploy keys → Add deploy key</strong>.</li>
        <li>Title: <code>easypanel</code>. Key: pegá la clave que copiaste.</li>
        <li><strong>Dejá SIN tildar “Allow write access”</strong> — EasyPanel solo necesita leer. Add key.</li>
      </ol>
      <div class="note warn">
        <span class="ic">🔒</span>
      ...
      <ol class="steps">
        <li>En EasyPanel, dentro de <strong>Source</strong> / <strong>Deployments</strong>, copiá la <strong>Webhook URL</strong> que ofrece el servicio.</li>
      ...
      <p>GitHub manda un ping de prueba: en la lista de webhooks debería aparecer un <strong>✓ verde</strong> (Recent Deliveries → response 200). Si aparece rojo, revisá que la URL esté completa.</p>
      <div class="note">
        <span class="ic">🔁</span>
        <div><p>A partir de acá: cada <code>git push origin main</code> dispara el webhook → EasyPanel clona con la deploy key → construye la imagen Docker → redespliega. Cero pasos manuales.</p></div>
      </div>
    </section>
    ...
    <section class="step" id="s5">
      ...
      <p>EasyPanel puede usar Nixpacks o buildpacks, pero acá mandamos nosotros: build method <strong>Dockerfile</strong>. Así el entorno de producción es idéntico al que definimos, sin sorpresas.</p>
      ...
      <ol class="steps">
        ...
        <li>Dockerfile path: <code>Dockerfile</code> (raíz del repo). Guardá.</li>
      </ol>
...
<span class="tok-k">EXPOSE</span> 80</code></pre>
      ...
      <div class="note">
        <span class="ic">📦</span>
        <div><p><strong>Si más adelante sumás Composer</strong>, agregá antes del <code>EXPOSE</code>: <code>COPY --from=composer:2 /usr/bin/composer /usr/bin/composer</code> y <code>RUN composer install --no-dev --optimize-autoloader</code>, y cambiá el <code>require</code> de <code>index.php</code> a <code>vendor/autoload.php</code>.</p></div>
      </div>
...
<span class="tok-c">// App\ -&gt; app/ . Si sumás librerías, pasá a Composer.</span>
...
&lt;/html&gt;</code></pre>
    ...
    <section class="step" id="s6">
      ...
      <div class="note">
        <span class="ic">💡</span>
        <div><p>La wiki (home <code>/</code>) abre <strong>sin</strong> base de datos. Postgres solo lo necesita la ruta <code>/demo</code> para mostrar el estado de conexión. Podés saltar 6.1 y 6.2 y sumarlo después.</p></div>
      </div>
      <ol class="steps">
      ...
      <p>En el servicio <code>web</code> → pestaña <strong>Environment</strong>, cargá las variables que lee <code>Database.php</code>. Usá los valores internos del servicio Postgres:</p>
      <table class="kv">
      ...
      <h3>6.3 · Desplegar</h3>
      <ol class="steps">
        <li>En el servicio <code>web</code>, tocá <strong>Deploy</strong> (o simplemente hacé un <code>git push</code> — el webhook lo dispara).</li>
        <li>Mirá los <strong>Logs / Deployments</strong>: verás el build de Docker (FROM, extensiones, copy del código) y luego el contenedor arriba.</li>
        <li>Abrí el <strong>dominio</strong> asignado en Domains → instancia real: <a href="https://apps-wiki.u1xuyr.easypanel.host/" target="_blank" rel="noopener">apps-wiki.u1xuyr.easypanel.host</a></li>
      </ol>
      ...
      <div class="divider-final">
        ...
        <p>La app quedó en producción y el pipeline en marcha. De acá en más el ciclo es: <strong>editás → <code>git push origin main</code> → EasyPanel construye y redespliega solo</strong>. Para una app nueva, repetís del paso 1 al 6 cambiando el nombre del repo y el <code>Dockerfile</code>.</p>
      </div>
...
<script>
  ...
  var cur = 'system';
  try { cur = localStorage.getItem('ep-theme') || 'system'; } catch (e) {}
  apply(cur);
  btn.addEventListener('click', function () {
